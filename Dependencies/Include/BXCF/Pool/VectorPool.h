...
template <class T>
bool				bxcf::VectorPool<T>::isEntryAtIndex(uint32 uiEntryIndex)
{
	return uiEntryIndex < m_vecEntries.size();
}

template <class T>
uint32				bxcf::VectorPool<T>::getEntryCount(void)
{
	return m_vecEntries.size();
}

template <class T>
T					bxcf::VectorPool<T>::getEntryByIndex(uint32 uiEntryIndex)
{
...
template <class T>
void				bxcf::VectorPool<T>::removeAllEntries(void)
{
...
template <class T>
void				bxcf::VectorPool<T>::addEntry(T pEntry)
{
	m_vecEntries.push_back(pEntry);
}

template <class T>
void				bxcf::VectorPool<T>::addEntryAtPosition(T pEntry, uint32 uiPosition)
{
	m_vecEntries.insert(m_vecEntries.begin() + uiPosition, pEntry);
}

template <class T>
void				bxcf::VectorPool<T>::removeEntry(T pEntry)
{
...
template <class T>
uint32				bxcf::VectorPool<T>::getNextEntryIndex(void)
{
	return m_vecEntries.size();
}

template <class T>
T					bxcf::VectorPool<T>::getFirstEntry(void)
{
	return m_vecEntries[0];
}

template <class T>
T					bxcf::VectorPool<T>::getLastEntry(void)
{
	return m_vecEntries[m_vecEntries.size() - 1];
}

template <class T>
uint32				bxcf::VectorPool<T>::getIndexByEntry(T pEntry)
{
...
template <class T>
void				bxcf::VectorPool<T>::setEntryByIndex(uint32 uiEntryIndex, T pEntry)
{
	m_vecEntries[uiEntryIndex] = pEntry;
}

template <class T>
void				bxcf::VectorPool<T>::swapEntries(T pEntry1, T pEntry2)
{
...
template <class T>
void				bxcf::VectorPool<T>::moveEntryToPosition(T pEntry, uint32 uiPosition)
{
