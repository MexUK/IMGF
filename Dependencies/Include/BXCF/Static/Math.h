...
public:
	...
	static uint32				getColumnIndex(Vec2i& vecPoint, Vec2i& vecMinPosition, float32 fRowWidth, uint32 uiColumnCount);
...
uint32				bxcf::Math::convertSectorsToBytes(uint32 uiSectorCount)
{
	return uiSectorCount * SECTOR_BYTE_COUNT;
}

uint32				bxcf::Math::convertBytesToSectors(uint32 uiByteCount)
{
