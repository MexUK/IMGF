...
template <class T>
static std::vector<T>								bxcf::StdVector::combineVectors(std::vector<T>& vecVector1, std::vector<T>& vecVector2)
{
	uint32 i = 0;
	vector<T> vecValues;
...
template <typename T>
std::vector<std::string>							bxcf::StdVector::mapSorted(std::vector<T>& vecVector, std::string(*fpCallback)(T))
{
