...
class bxa::CDATPathFormat : public bxa::CFormat
{
public:
	CDATPathFormat(void);
	...
	void												setPathsFormat(bxa::eDATPathFormat eFormat) { m_header.m_ePathsFormat = eFormat; }
	bxa::eDATPathFormat										getPathsFormat(void) { return m_header.m_ePathsFormat; }

	void												applyOffsetToPositions(bxa::CVector3D& vecPositionOffset);
	...
	bxa::CDATEntry_Paths_General_Header&						getHeader(void) { return m_header; }
...
private:
	...
	bxa::eDATPathFormat										detectPathsFormat(void);

public: // todo - make this access specifier private
	uint32												m_uiFileIndex;
	bxa::CDATEntry_Paths_General_Header						m_header;
	std::vector<bxa::CDATEntry_Paths_General_PathNode>		m_vecPathNodes;
	std::vector<bxa::CDATEntry_Paths_General_NaviNode>		m_vecNaviNodes;
	std::vector<bxa::CDATEntry_Paths_General_Link>			m_vecLinks;
};
