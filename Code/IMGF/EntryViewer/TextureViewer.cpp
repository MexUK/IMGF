...
void				TextureViewer::loadThreadAndWindow(void)
{
...
void				TextureViewer::unloadThreadAndWindow(void)
{
	removeAllEntries();
...
{
	SetWindowText(getWindowHwnd(), String::convertStdStringToStdWString(getWindowTitle()).c_str());
}

DWORD WINAPI		TextureViewer::onThreadStarted(LPVOID lpParam)
{
	TextureViewer *pTextureViewer = (TextureViewer*)lpParam;
	for(;;)
	{
...
void				TextureViewer::processWindow(void)
{
	MSG Msg;
...
void				TextureViewer::prepareRenderData(void)
{
...
void				TextureViewer::openWindow(void)
{
	// init settings
	setZoomLevel(1.0f);
...
void				TextureViewer::closeWindow(void)
{
	setWindowIsOpen(false);
...
void				TextureViewer::repositionWindow(void)
{
	/*
	todo
...
void				TextureViewer::initDisplayType(void)
{
	if (getDisplayType() == DISPLAYTYPE_SINGLE)
	{
		...
		{
			((CComboBox*)CWnd::FromHandle(hWndComboBox))->InsertString(i, String::convertStdStringToStdWString(String::toString(uiZoomLevels[i]) + "%").c_str());
		}
...
void				TextureViewer::uninitDisplayType(void)
{
	if (getDisplayType() == DISPLAYTYPE_SINGLE)
	{
...
LRESULT CALLBACK	WndProc_EntryViewer(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		TextureViewerTextureData *pActiveImageData = nullptr;
	...
	case WM_KEYDOWN:
	{
		...
		TextureViewerTextureData *pTexData;
...
void			TextureViewer::forceRender(void)
{
	RECT rect;
...
void				TextureViewer::clearWindowBackground(HDC hdc)
{
	RECT clientRect;
...
void			TextureViewer::render(void)
{
...
void			TextureViewer::renderDisplayType_Single(void)
{
	HWND hwnd = getWindowHwnd();
	...
	for (auto pImageData : getIMGF()->getEntryViewerManager()->getTextureViewer()->getEntries())
	{
		...
		if (false)
		{
			...
			//Debugger::log("IMAGE X = " + String::toString(uiImageX));
	...
	rect.right = 8000;
	rect.bottom = 8000;
	DrawText(hdc, String::convertStdStringToStdWString(pActiveImageData->m_strDiffuseName).c_str(), pActiveImageData->m_strDiffuseName.length(), &rect, DT_NOPREFIX); // diffuse name

	rect.left = g_uiLeftPanelWidth + 5;
	rect.top = 25;
	rect.right = 8000;
	rect.bottom = 8000;
	DrawText(hdc, String::convertStdStringToStdWString(pActiveImageData->m_strAlphaName).c_str(), pActiveImageData->m_strAlphaName.length(), &rect, DT_NOPREFIX); // alpha name

	rect.left = g_uiLeftPanelWidth + 120 + 5;
	rect.top = 6;
	rect.right = 8000;
	rect.bottom = 8000;
	string strText = String::toString(pActiveImageData->m_uiWidth) + " x " + String::toString(pActiveImageData->m_uiHeight);
	DrawText(hdc, String::convertStdStringToStdWString(strText).c_str(), strText.length(), &rect, DT_NOPREFIX); // size
	...
	rect.right = 8000;
	rect.bottom = 8000;
	strText = "0x" + String::toStringHex(pActiveImageData->m_uiRasterFormat);
	DrawText(hdc, String::convertStdStringToStdWString(strText).c_str(), strText.length(), &rect, DT_NOPREFIX); // raster format
	*/
...
void			TextureViewer::renderDisplayType_Float(void)
{
	HWND hwnd = getWindowHwnd();
